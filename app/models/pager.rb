class Pager
  ...
  attr_accessor :previous_page, :next_page, :range, :count, :num_pages
  ...
  def Pager.class_pages(paged_class, page=1, page_size=10, options={ })
    all = paged_class.find(:all, :conditions => options[:query], :order => options[:order], :include => options[:includes], :offset => (page.to_i-1) * page_size, :limit => page_size)
    length = paged_class.count(:conditions => options[:query])
